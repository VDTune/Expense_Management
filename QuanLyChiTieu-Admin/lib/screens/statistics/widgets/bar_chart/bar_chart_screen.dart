...
class Transaction {
  ...
  factory Transaction.fromDocument(DocumentSnapshot doc) {
    final data = doc.data() as Map<String, dynamic>?; // Chuyển đổi dữ liệu tài liệu sang Map
    ...
    return Transaction(
      monthYear: data.containsKey('monthyear') ? data['monthyear'] as String : '',
      remainingAmount: data.containsKey('remainingAmount')
          ? (data['remainingAmount'] as num).toDouble()
          : 0.0,
      totalCredit: data.containsKey('totalCredit')
          ? (data['totalCredit'] as num).toDouble()
          : 0.0,
      totalDebit: data.containsKey('totalDebit')
